...
const EditUser = () => {
  ...
  const submit = (data) => {
    ...
    const { firstName, lastName, email, role, rooms, occupantRoomIds, phones, phoneIds, clubAdvisor, clubs, emergencyPhone, emergencyEmail, TA, RA, undergraduate, graduate, officeHours, position, picture, interests, clubAdvisorIds, clubIds, interestIds } = data;
    console.log(data);
    let phonesArray;
    let collectionName;
    ...
    case 'FACULTY':
      ...
      updateData = { id: _id, email, firstName, lastName, officeHours, position, picture, emergencyPhone, emergencyEmail, rooms, occupantRoomIds, phones: phonesArray, phoneIds, clubs, clubIds, clubAdvisorIds, clubAdvisor };
      updateMethod.callPromise({ collectionName, updateData })
        .catch(error => swal('Error', error.message, 'error'))
        .then(() => {

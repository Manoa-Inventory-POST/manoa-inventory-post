...
class StudentProfileCollection extends BaseProfileCollection {
  ...
  checkIntegrity() {
    ...
    this.find().forEach((doc) => {
      if (doc.role !== ROLE.STUDENT) {
        problems.push(`StudentProfile instance does not have ROLE.USER: ${doc}`);
      }
    });
    return problems;
  }
  ...
  dumpOne(docID) {
    ...
    return { email, firstName, lastName, ta, ra, undergraduate, graduate }; // CAM this is not enough for the define method. We lose the password.
  }
}
